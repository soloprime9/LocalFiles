...
# Stage 1 is just brainstorming (no strict schema needed) — using a lighter
# model here spreads token usage across TWO separate free-tier quota buckets
# instead of hammering one model's limit twice per analysis.
STAGE1_MODEL = "gemini-flash-lite-latest"
STAGE2_MODEL = "gemini-flash-latest"


def call_with_retry(fn, max_retries=3):
    """Wraps a Gemini API call — on 429 (rate limit), waits the time Google
    tells us to (from the error itself) and retries, instead of crashing."""
    for attempt in range(max_retries):
        try:
            ...
            if e.code == 429 and attempt < max_retries - 1:
                ...
                if m:
                    wait = float(m.group(1)) + 2  # +2s buffer
