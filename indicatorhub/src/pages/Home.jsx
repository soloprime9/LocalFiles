...
export default function Home() {
  ...
  const { trending, featured, stats, categories } = state;
  ...
  useEffect(() => {
    ...
    // Stats (fallback if context hasn't loaded)
    if (!stats) {
      getStats()
        .then((r) => {
          const data = r?.data || r;
          setLocalStats(data || FALLBACK_STATS);
          dispatch({ type: ACTIONS.FETCH_STATS_SUCCESS, payload: data || FALLBACK_STATS });
        })
  ...
  const displayCategories = categories?.length ? categories : localCategories;
