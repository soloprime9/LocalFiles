...
#         header = ttk.Label(main_frame, text="⚡ YouTube → Multiple Vertical Shorts (One Pass)", font=("Arial", 14, "bold"))
#         header.pack(pady=(0, 15), anchor="w")

#         # Section 1: Video Link
#         source_frame = ttk.LabelFrame(main_frame, text=" 1. Enter YouTube URL ", padding="10")
...
#         self.check_btn = ttk.Button(url_row, text="🔍 Check Duration", command=self.start_check_duration_thread)
#         self.check_btn.pack(side=tk.RIGHT)

#         self.duration_label = ttk.Label(source_frame, text="Video duration: not checked yet", foreground="#555555")
#         self.duration_label.pack(anchor="w", pady=(8, 0))
...
#         ttk.Label(auto_row, text="Clip length (sec):").pack(side=tk.LEFT, padx=(0, 5))
#         self.clip_len_spin = ttk.Spinbox(auto_row, from_=5, to=180, width=6)
#         self.clip_len_spin.set(30)
#         self.clip_len_spin.pack(side=tk.LEFT, padx=(0, 15))

#         ttk.Label(auto_row, text="Gap between clips (sec):").pack(side=tk.LEFT, padx=(0, 5))
#         self.gap_spin = ttk.Spinbox(auto_row, from_=0, to=600, width=6)
#         self.gap_spin.set(0)
#         self.gap_spin.pack(side=tk.LEFT, padx=(0, 15))

#         ttk.Label(auto_row, text="Max clips (0 = no limit):").pack(side=tk.LEFT, padx=(0, 5))
#         self.max_clips_spin = ttk.Spinbox(auto_row, from_=0, to=200, width=6)
#         self.max_clips_spin.set(0)
#         self.max_clips_spin.pack(side=tk.LEFT)
...
#             config_frame,
#             text='Example: a 2-minute video with a 30s clip length auto-generates 4 shorts (0-30, 30-60, 60-90, 90-120).\nDuration is detected automatically when you click "Build" — no need to check it first.',
#             foreground="#545CFF", justify="left"
#         )
#         auto_hint.pack(anchor="w", pady=(8, 0))
...
#             self.manual_frame,
#             text="Example:\n10-40\n75-100\n200-230",
#             foreground="#B325FF", justify="left"
#         )
#         manual_hint.pack(anchor="w", pady=(0, 5))

#         self.ranges_text = tk.Text(self.manual_frame, height=5, font=("Consolas", 10))
#         self.ranges_text.pack(fill=tk.X)
#         self.ranges_text.insert("1.0", "10-40\n75-100")
...
#         ttk.Label(quality_row, text="Max Quality:").pack(side=tk.LEFT, padx=(0, 5))
#         self.quality_combo = ttk.Combobox(quality_row, values=["1080", "720", "480"], width=6, state="readonly")
#         self.quality_combo.set("720")
#         self.quality_combo.pack(side=tk.LEFT, padx=(0, 20))

#         ttk.Label(quality_row, text="Encode Speed:").pack(side=tk.LEFT, padx=(0, 5))
#         self.speed_combo = ttk.Combobox(quality_row, values=["ultrafast", "veryfast", "fast"], width=10, state="readonly")
#         self.speed_combo.set("ultrafast")
#         self.speed_combo.pack(side=tk.LEFT)

#         # Section 3: Log Monitor
#         activity_frame = ttk.LabelFrame(main_frame, text=" 3. Live Pipeline Monitor (Step-by-Step) ", padding="10")
#         activity_frame.pack(fill=tk.BOTH, expand=True, pady=10)

#         self.log_box = ScrolledText(activity_frame, height=12, font=("Consolas", 10), bg="#1e1e1e", fg="#ffffff")
#         self.log_box.pack(fill=tk.BOTH, expand=True)
#         self.log_box.insert(tk.END, "[SYSTEM] Engine Ready. Input link + clip ranges, then press build.\n")
#         self.log_box.config(state=tk.DISABLED)
...
#         self.process_btn = ttk.Button(self.btn_frame, text="🚀 Build All Shorts Now", command=self.start_pipeline_thread)
#         self.process_btn.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

#         self.open_folder_btn = ttk.Button(self.btn_frame, text="📁 Open Shorts Folder", command=self.open_shorts_folder)
#         self.open_folder_btn.pack(side=tk.RIGHT, padx=5, expand=True, fill=tk.X)
...
#         # Section 2: Multiple time ranges
#         config_frame = ttk.LabelFrame(
#             main_frame,
#             text=" 2. Define Multiple Clips — one per line as  start-end  (seconds) ",
#             padding="10"
#         )
#         config_frame.pack(fill=tk.X, pady=5)
...
#         hint = ttk.Label(
#             config_frame,
#             text="Example:\n10-40\n75-100\n200-230\n\nTip: click 'Check Duration' first, then 'Auto-Fill Ranges' to split\nthe whole video into equal short clips automatically.",
#             foreground="#888888", justify="left"
#         )
#         hint.pack(anchor="w", pady=(0, 5))

#         self.ranges_text = tk.Text(config_frame, height=5, font=("Consolas", 10))
#         self.ranges_text.pack(fill=tk.X)
#         self.ranges_text.insert("1.0", "10-40\n75-100")

#         # Auto-fill row
#         autofill_row = ttk.Frame(config_frame)
#         autofill_row.pack(fill=tk.X, pady=(8, 0))

#         ttk.Label(autofill_row, text="Clip length (sec):").pack(side=tk.LEFT, padx=(0, 5))
#         self.clip_len_spin = ttk.Spinbox(autofill_row, from_=5, to=180, width=6)
#         self.clip_len_spin.set(30)
#         self.clip_len_spin.pack(side=tk.LEFT, padx=(0, 15))

#         ttk.Label(autofill_row, text="Gap between clips (sec):").pack(side=tk.LEFT, padx=(0, 5))
#         self.gap_spin = ttk.Spinbox(autofill_row, from_=0, to=600, width=6)
#         self.gap_spin.set(0)
#         self.gap_spin.pack(side=tk.LEFT, padx=(0, 15))

#         self.autofill_btn = ttk.Button(autofill_row, text="✨ Auto-Fill Ranges From Full Video", command=self.auto_fill_ranges)
#         self.autofill_btn.pack(side=tk.LEFT)

#         # Quality / speed selector
#         quality_row = ttk.Frame(config_frame)
#         quality_row.pack(fill=tk.X, pady=(10, 0))
#         ttk.Label(quality_row, text="Max Quality:").pack(side=tk.LEFT, padx=(0, 5))
#         self.quality_combo = ttk.Combobox(quality_row, values=["1080", "720", "480"], width=6, state="readonly")
#         self.quality_combo.set("720")
...
#         if not self.video_duration:
#             messagebox.showinfo("Duration Needed", "Click 'Check Duration' first so the full video length is known.")
#             return
...
#                 raise ValueError
#         except ValueError:
#             messagebox.showerror("Invalid Input", "Clip length must be a positive number of seconds.")
#             return
...
#         if not ranges:
#             messagebox.showinfo("No Clips", "Video is shorter than the chosen clip length.")
#             return

#         text = "\n".join(f"{s}-{e}" for s, e in ranges)
#         self.ranges_text.delete("1.0", tk.END)
#         self.ranges_text.insert("1.0", text)
#         self.log_message(f"[INFO] Auto-filled {len(ranges)} clip range(s) covering the full {fmt_duration(total)} video.")
...
#             ffmpeg_exe_path = imageio_ffmpeg.get_ffmpeg_exe()
#             self.log_message(f"[SYSTEM] Using FFmpeg binary: {Path(ffmpeg_exe_path).name}")
#             self.log_message(f"[STEP 1/2] Resolving direct stream URLs (resolved ONCE, reused for all {len(ranges)} clips)...")
