...
# zoom na ho to yahan koi scale filter hi nahi lagta — pixels bilkul native rehte hain
    ...
    for t in texts:
        ...
        fc.append(f"[{cur}]drawtext=text='{esc}':fontsize={size}:fontcolor='#{color}':x={tx}:y={ty}{box}{font_opt}{enable_opt}[{nxt}]")
        cur = nxt
  ...
  while(!done){
    ...
    sd.clips.forEach(c=>{
      if (!processedClipIds.has(c.clip_id)) {
        ...
        // Opt-in only (checkbox defaults to OFF): silently remove dead air right
        // after this clip is cut, before it's queued for export. Still fully
        // undoable per-clip afterward from the Speed/Zoom or Export tab.
        ...
        if (autoDeadAirOn) {
          ...
          autoApplyDeadAirSilently(c.clip_id).finally(queueForExport);
        } else {
          queueForExport();
        }
      }
...
async function importCaptions(){
  ...
  (data.cues || []).forEach(cue => {
    ...
    const layer = {
      ...
      font: 'default', style: 'classic_box', boxColor: '#000000', enabled: true,
      start: Math.max(0, localStart),
      end: clipDur !== null ? Math.min(clipDur, localEnd) : localEnd
    };
    textLayers.push(layer);
    renderTextPanelRow(layer);
    added++;
  ...
  document.getElementById('capLangWrap').classList.add('hidden');
  setCapStatus(added ? `✅ Imported ${added} caption line(s) as timed text layers.` : '⚠️ None of the fetched captions fall inside this clip\'s trimmed range.', added === 0);
}
...
async function saveVideo(){
  ...
  const settings = {
    ...
    mute: amode === 'mute',
    texts: textLayers.filter(l=>l.enabled).map(l=>({content:l.content, x:l.x, y:l.y, size:l.size, color:l.color, box:l.box, font:l.font, style:l.style, box_color:l.boxColor, start:l.start, end:l.end})),
