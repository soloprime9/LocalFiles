...
def get_model(model_size):
    ...
    with _MODEL_LOCK:
        if model_size not in _MODEL_CACHE:
            already_cached = (Config.MODEL_CACHE_DIR / f"models--Systran--faster-whisper-{model_size}").exists()
            logger.info(
                f"{'Loading' if already_cached else 'Downloading (one-time only) and loading'} "
                f"model '{model_size}'..."
            )
            ...
            _MODEL_CACHE[model_size] = BatchedInferencePipeline(model=base_model)
...
if __name__ == '__main__':
    ...
    logger.info("Ready.")
    app.run(host="0.0.0.0", port=80867, debug=False, threaded=True)