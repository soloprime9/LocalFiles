...
def build_and_run_export(src, w, h, s, title):
    ...
    for t in texts:
        ...
        fc.append(f"[{cur}]drawtext=text='{esc}':fontsize={size}:fontcolor=0x{color}:x={tx}:y={ty}{box}[{nxt}]")
        cur = nxt
...
async function saveVideo(){
  ...
  const settings = {
    ...
    mute: amode === 'mute',
    texts: textLayers.map(l=>({content:l.content, x:l.x, y:l.y, size:l.size, color:l.color, box:l.box})),
    logo: logoUrl ? {
