...
def compute_auto_ranges(total_dur, clip_len, mode="auto"):
    ranges = []
    t = 0
    if mode == "auto_10s":
        # 10s segments with 1s gap: 0-10, 11-21, 22-32, etc.
        while t + 5 < total_dur:
            e = min(t + 10, total_dur)
...
def _run_cut_job(job_id, url, height, mode, clip_len, manual_ranges):
    ...
    if mode == "auto":
        if not total_dur:
            ...
            job["done"] = True
            return
        ranges = compute_auto_ranges(total_dur, clip_len, mode="auto")
    elif mode == "auto_10s":
        if not total_dur:
            job["error"] = "Could not detect duration for auto mode"
            job["done"] = True
            return
        ranges = compute_auto_ranges(total_dur, 10, mode="auto_10s")
    else:
...
<div class="wrap">
  ...
  <div class="card" id="fetchCard">
    <div class="row">
      ...
      <div style="min-width:140px;">
        <label>Mode</label>
        <select id="mode" onchange="toggleMode()"><option value="auto">Auto-split</option><option value="auto_10s">Auto Clip (10s segments, 1s gap)</option><option value="manual">Manual ranges</option></select>
      </div>
...
30-60"></textarea>
  ...
  <!-- Bulk Automation & Anti-Copyright Card -->
  <div class="card hidden" id="automationCard">
    <h3 style="margin:0 0 10px 0; background:var(--grad); -webkit-background-clip:text; background-clip:text; color:transparent; font-weight:800;">🤖 1-Click Bulk Automation &amp; Anti-Copyright Tool</h3>
    <p class="sub" style="margin-top:0; margin-bottom:18px;">Apply anti-copyright protection algorithms and professional adjustments to ALL shorts, and bulk-export them one-by-one right to your computer.</p>
    
    <div class="row" style="gap: 18px; align-items: flex-start; margin-bottom: 20px;">
      <div style="flex:1; min-width:220px;">
        <label>⚙️ Anti-Copyright Presets</label>
        <div class="checkrow"><input type="checkbox" id="bulkHFlip" checked> Rotate &amp; Mirror Video (Horizontal Flip)</div>
        <div class="checkrow"><input type="checkbox" id="bulkAutoZoom" checked> Small Anti-Crop Zoom (1.10x)</div>
        <div class="checkrow"><input type="checkbox" id="bulkAutoSpeed" checked> Slight Speed Alteration (1.05x speed)</div>
        <div class="checkrow"><input type="checkbox" id="bulkEnhance" checked> HD Enhance (Denoise &amp; Sharpen)</div>
      </div>
      
      <div style="flex:1; min-width:200px;">
        <label>🎨 Quick Color Look</label>
        <select id="bulkColorPreset">
          <option value="vivid" selected>Vivid Pop (Recommended)</option>
          <option value="cinematic">Cinematic</option>
          <option value="vintage_vhs">Vintage VHS Look</option>
          <option value="moody">Moody</option>
          <option value="none">None</option>
        </select>
        
        <label style="margin-top:10px;">🏷️ Overlay Branding Brand Logo</label>
        <input type="file" id="bulkLogoFile" accept="image/*" onchange="uploadBulkLogo()">
        <div id="bulkLogoStatus" class="sub" style="margin-top:5px; color:var(--accent2);"></div>
      </div>
      
      <div style="flex:1; min-width:200px;">
        <label>🎵 Bulk Background Audio</label>
        <select id="bulkAudioSelect" onchange="toggleBulkAudio()">
          <option value="none" selected>Keep Original Audio</option>
          <option value="audio_library">Mix with Audio Library MP3</option>
          <option value="upload">Mix with custom MP3 upload</option>
        </select>
        <div id="bulkAudioLibraryWrap" class="hidden" style="margin-top:8px;">
          <select id="bulkAudioLibChoice" style="width: 100%;"><option value="">No audio library files loaded</option></select>
        </div>
        <div id="bulkAudioUploadWrap" class="hidden" style="margin-top:8px;">
          <input type="file" id="bulkAudioUpload" accept="audio/*" onchange="uploadBulkAudio()">
          <div id="bulkAudioUploadStatus" class="sub" style="margin-top:5px; color:var(--accent2);"></div>
        </div>
      </div>
    </div>
    
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; border-top:1px solid var(--border); padding-top:15px;">
      <div>
        <span class="badge" style="background:rgba(34,211,196,0.15); color:var(--accent2);">Ready for 1-Click Automation</span>
      </div>
      <button class="btn-grad" id="bulkExportBtn" style="font-size:16px; padding:12px 24px; display:flex; align-items:center; gap:8px;" onclick="runBulkAutomation()">
        <span>🚀 Bulk Process &amp; Export All Shorts</span>
      </button>
    </div>
  </div>
  
  <!-- Exported Downloads Card -->
  <div class="card hidden" id="downloadListCard">
    <h3 style="margin:0 0 10px 0; color:var(--accent2); font-weight:800;">📥 Exported Downloads List</h3>
    <p class="sub" style="margin-top:0; margin-bottom:14px;">All exported video shorts will appear here line-by-line. Hit Download to save directly to your PC.</p>
    
    <div style="display:flex; flex-direction:column; gap:10px;" id="downloadList">
      <!-- Line by Line Download Rows -->
    </div>
  </div>
...
function addClipCard(c){
  ...
  allLoadedClips.push(c);
}

async function fetchAndCut(){
  allLoadedClips = [];
  document.getElementById('downloadListCard').classList.add('hidden');
  ...
  document.getElementById('automationCard').classList.add('hidden');
...
async function loadAudioLibrary(){
  ...
  if(audioLibCache){ renderAudioLibrary(); populateBulkAudioLibOptions(); return; }
  const res = await fetch('/api/audio_library');
  const data = await res.json();
  audioLibCache = data.files || [];
  renderAudioLibrary();
  populateBulkAudioLibOptions();
}
...
function populateBulkAudioLibOptions() {
  const select = document.getElementById('bulkAudioLibChoice');
  if(!select) return;
  select.innerHTML = '<option value="">-- Select background music --</option>';
  if(audioLibCache && audioLibCache.length) {
    audioLibCache.forEach(f => {
    ...
    select.innerHTML = '<option value="">No audio library files available</option>';
  }
}

function toggleBulkAudio() {
  const mode = document.getElementById('bulkAudioSelect').value;
  document.getElementById('bulkAudioLibraryWrap').classList.toggle('hidden', mode !== 'audio_library');
  document.getElementById('bulkAudioUploadWrap').classList.toggle('hidden', mode !== 'upload');
}

async function uploadBulkLogo() {
  const f = document.getElementById('bulkLogoFile').files[0];
  if(!f) return;
  const status = document.getElementById('bulkLogoStatus');
  status.innerText = "⏳ Uploading brand logo...";
  const fd = new FormData();
  fd.append('file', f);
  try {
    const res = await fetch('/api/upload', {method:'POST', body: fd});
    const data = await res.json();
    bulkLogoUrl = data.url;
    status.innerText = "✅ Brand logo uploaded successfully: " + f.name;
  } catch(e) {
...
async function uploadBulkAudio() {
  const f = document.getElementById('bulkAudioUpload').files[0];
  if(!f) return;
  const status = document.getElementById('bulkAudioUploadStatus');
  status.innerText = "⏳ Uploading audio tracks...";
  const fd = new FormData();
  fd.append('file', f);
  try {
    const res = await fetch('/api/upload', {method:'POST', body: fd});
    const data = await res.json();
    bulkAudioUrl = data.url;
    status.innerText = "✅ Background audio uploaded: " + f.name;
  } catch(e) {
    status.innerText = "❌ Upload failed: " + e.message;
  }
}

async function runBulkAutomation() {
  if (!allLoadedClips.length) {
    alert("No clips loaded yet. Fetch some YouTube videos first!");
    return;
  }
  
  const bulkBtn = document.getElementById('bulkExportBtn');
  bulkBtn.disabled = true;
  bulkBtn.innerHTML = "<span>⏳ Processing Bulk Queue...</span>";
  
  const dListCard = document.getElementById('downloadListCard');
  dListCard.classList.remove('hidden');
  const dList = document.getElementById('downloadList');
  dList.innerHTML = ''; // Fresh list
  
  // Base settings to override for each clip
  const resVal = document.getElementById('resolution').value;
  const fmtVal = document.getElementById('format').value;
  const crfVal = parseInt(document.getElementById('crf').value);
  const presetVal = document.getElementById('preset').value;
  
  // Step through clips one by one sequential queue, avoiding rearrangement
  for (let idx = 0; idx < allLoadedClips.length; idx++) {
    const clip = allLoadedClips[idx];
    
    // Create status row
    const rowId = "bulk_row_" + clip.clip_id;
    const rowHtml = `
      <div id="${rowId}" style="background:var(--panel2); border:1px solid var(--border); border-radius:12px; padding:12px 16px; display:flex; flex-direction:column; gap:8px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <div style="font-weight:600; color:var(--text);">${clip.label}</div>
          <div class="bulk-status-badge" style="font-size:12px; font-weight:700; color:var(--warn);">⏳ Waiting...</div>
        </div>
        <div class="progress-container" style="margin: 0; background:var(--panel);">
          <div class="progress-fill" style="width: 0%;" id="bulk_fill_${clip.clip_id}"></div>
          <div class="progress-text" id="bulk_text_${clip.clip_id}">0%</div>
        </div>
      </div>
    `;
    dList.insertAdjacentHTML('beforeend', rowHtml);
  }
  
  // Run process one by one
  for (let idx = 0; idx < allLoadedClips.length; idx++) {
    const clip = allLoadedClips[idx];
    const rowEl = document.getElementById("bulk_row_" + clip.clip_id);
    const badgeEl = rowEl.querySelector(".bulk-status-badge");
    const fillEl = document.getElementById(`bulk_fill_${clip.clip_id}`);
    const textEl = document.getElementById(`bulk_text_${clip.clip_id}`);
    
    badgeEl.innerText = "⚙️ Exporting...";
    badgeEl.style.color = "var(--accent)";
    
    const settings = {
      speed: document.getElementById('bulkAutoSpeed').checked ? 1.05 : 1.0,
      zoom: document.getElementById('bulkAutoZoom').checked ? 1.10 : 1.0,
      pan_x: 0, pan_y: 0,
      contrast: 1.0, saturation: 1.0, brightness: 0.0,
      sharpen: document.getElementById('bulkEnhance').checked,
      enhance: document.getElementById('bulkEnhance').checked,
      color_preset: document.getElementById('bulkColorPreset').value,
      resolution: resVal,
      format: fmtVal,
      crf: crfVal,
      preset: presetVal,
      hflip: document.getElementById('bulkHFlip').checked,
      regions: [],
      audio_mode: 'original',
      logo: null,
      texts: []
    ...
    const bulkAudioMode = document.getElementById('bulkAudioSelect').value;
    if (bulkAudioMode === 'audio_library') {
      const val = document.getElementById('bulkAudioLibChoice').value;
      if (val) {
        settings.audio_mode = 'replace';
        settings.audio_file_url = val;
      }
    } else if (bulkAudioMode === 'upload' && bulkAudioUrl) {
      settings.audio_mode = 'replace';
      settings.audio_file_url = bulkAudioUrl;
    }
    ...
    try {
      const res = await fetch('/api/export', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({clip_id: clip.clip_id, settings})
      });
      const data = await res.json();
      if (data.error) {
        badgeEl.innerText = "❌ Failed: " + data.error;
      ...
      while(!done) {
        ...
        const sd = await statusRes.json();
        if (sd.error) {
          badgeEl.innerText = "❌ Failed: " + sd.error;
          badgeEl.style.color = "var(--danger)";
          done = true;
          break;
        }
        ...
        if (sd.status === 'done') {
          ...
          badgeEl.style.color = "var(--accent2)";
          
          // Add a download button to the row
          const actionsDiv = document.createElement('div');
          actionsDiv.style = "display:flex; justify-content:flex-end; gap:10px; margin-top:8px;";
          actionsDiv.innerHTML = `
            <a class="btn-grad dl-link" href="${sd.url}" target="_blank" style="padding: 6px 14px; font-size:12px; border-radius:6px; text-decoration:none; color:#0c0d13; display:inline-block; font-weight:700;">
              📥 Download Short to PC
            </a>
          `;
      ...
      badgeEl.innerText = "❌ Error: " + e.message;
      badgeEl.style.color = "var(--danger)";
    }
  }
  
  bulkBtn.disabled = false;
  bulkBtn.innerHTML = "<span>🚀 Bulk Process &amp; Export All Shorts</span>";
}
</script>
</body>
</html>
