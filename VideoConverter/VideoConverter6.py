...
def run_extraction_job(job_id: str, extractor: VideoExtractor):
    ...
    try:
        ...
        # The raw uploaded video (if any) is only needed while OpenCV is
        # reading frames from it — the extracted frames already live in
        # PREVIEW_DIR, so the original upload can be discarded now.
        with jobs_lock:
            upload_dir = jobs.get(job_id, {}).get("upload_dir")
        if upload_dir:
...
def extract():
    ...
    # The frontend now always posts multipart/form-data (FormData), so a
    # locally uploaded file can travel alongside the plain text fields.
    ...
    if uploaded_file and uploaded_file.filename:
        # Save the uploaded video into its own temp folder. The user never
        # types or sees a filesystem path — they just picked a file, and
        # the browser streamed its bytes to us.
        upload_dir = Path(tempfile.mkdtemp(prefix="upload_"))
        safe_name = "".join(
            c for c in uploaded_file.filename if c.isalnum() or c in (' ', '_', '-', '.')
        ).strip() or "uploaded_video"
        saved_path = upload_dir / safe_name
        uploaded_file.save(str(saved_path))
        source = str(saved_path)
    elif not source:
        return jsonify({"error": "Please provide a video URL or choose a file to upload."}), 400
...
<style>
  * { box-sizing: border-box; }
  ...
  /* --- Source picker: "Paste URL" / "Upload from PC" dark twin boxes --- */
  .source-card {
    background: #0c0e1a;
  ...
  .source-icon {
    ...
    background: linear-gradient(135deg, #6a5cf5, #34c3e8);
    display: flex; align-items: center; justify-content: center;
    font-size: 20px; margin-bottom: 14px;
    box-shadow: 0 4px 14px rgba(88,101,242,0.35);
  }
  .source-title { color: #fff; font-size: 16px; font-weight: 700; margin-bottom: 6px; }
  .source-desc { color: rgba(255,255,255,0.5); font-size: 12.5px; line-height: 1.5; margin-bottom: 18px; }
  .source-box #source {
    width: 100%; padding: 12px 14px; border-radius: 10px;
    border: 1px solid rgba(255,255,255,0.12); background: #191c2c;
    color: #eaeaf5; font-size: 13.5px; outline: none;
  }
  .source-box #source::placeholder { color: rgba(255,255,255,0.35); }
  .source-box #source:focus { border-color: #6a5cf5; }
  .source-box:has(#source:disabled) { opacity: .45; }

  .choose-file-btn {
    width: 100%; padding: 13px; border: none; border-radius: 10px;
    font-size: 14.5px; font-weight: 700; color: #0b0d14; cursor: pointer;
    background: linear-gradient(90deg, #6a5cf5, #4aa9f0, #34e0c9);
    margin-top: auto;
    transition: transform .12s ease, box-shadow .12s ease;
  }
  .choose-file-btn:hover { transform: translateY(-1px); box-shadow: 0 4px 16px rgba(74,169,240,0.4); }
  .file-name { color: #34e0c9; font-size: 12.5px; margin-top: 10px; word-break: break-all; }
  ...
  .interval-label {
    color: rgba(255,255,255,0.6); font-size: 13px; display: flex;
    align-items: center; gap: 8px;
  }
  .interval-label input[type=number] {
    width: 64px; padding: 9px; border: 1px solid rgba(255,255,255,0.15);
    border-radius: 8px; font-size: 13px; background: #191c2c; color: #eaeaf5;
  }
  .extract-row #extractBtn {
    padding: 13px 36px; font-size: 15px;
  }

  @media (max-width: 720px) {
    .source-row { flex-direction: column; }
    .source-divider { padding: 10px 0; }
  }

  .meta-line { font-size: 13px; color: #555; margin-top: 10px; }
  .source-card .meta-line { color: rgba(255,255,255,0.65); text-align: center; }
  .source-card .meta-line#folderLine { color: #9b8cff !important; }
  .badge {
  ...
  <div class="source-card">
    <div class="source-row">
      <div class="source-box">
        ...
        <div class="source-title">Paste Video URL</div>
        <div class="source-desc">YouTube, Instagram, TikTok, Facebook, X/Twitter, Vimeo &amp; 1000+ more sites</div>
        <input type="text" id="source" placeholder="https://youtu.be/..." oninput="clearFileSelection()">
      </div>
      ...
      <div class="source-box">
        <div class="source-icon">📁</div>
        <div class="source-title">Upload from PC / Device</div>
        <div class="source-desc">Already have the video? Use it directly — stays on this device, no external server</div>
        <input type="file" id="fileInput" accept="video/*" style="display:none;" onchange="handleFileSelect()">
        <button type="button" class="choose-file-btn" onclick="el('fileInput').click()">Choose File</button>
        <div class="file-name" id="fileNameLabel"></div>
      </div>
    </div>

    <div class="extract-row">
      <label class="interval-label">Frame every
    ...
    <div class="meta-line" id="folderLine"></div>
  </div>
...
function handleFileSelect() {
  ...
  if (!f) return;
  selectedFile = f;
  el('fileNameLabel').innerText = '✓ ' + f.name;
  // Picking a local file and typing a URL are mutually exclusive.
  el('source').value = '';
...
async function startExtraction() {
  ...
  resetUI();
  el('statusText').innerText = selectedFile ? 'Uploading & starting extraction…' : 'Starting extraction…';
